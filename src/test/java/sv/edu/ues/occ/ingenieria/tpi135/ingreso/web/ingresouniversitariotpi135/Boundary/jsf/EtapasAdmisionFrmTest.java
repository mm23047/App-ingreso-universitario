...
class EtapasAdmisionFrmTest {

    @InjectMocks private EtapasAdmisionFrm frm;
    ...
    void setUp() {
        ...
        entidad.setNombre("Etapa Básica");
        entidad.setDescripcion("Primera etapa");
    ...
    @Test
    void constructor_NombreBeanEsEtapasDeAdmision() {
        assertEquals("Etapas de Admisión", frm.getNombreBean());
    }
    ...
    void getIdAsText_EntidadSinId_RetornaNull() {
        assertNull(frm.getIdAsText(new EtapasAdmision()));
    }
    ...
    void getIdByText_IdValido_RetornaEntidadDesdeDAO() {
        when(etapasAdmisionDAO.leer(testId)).thenReturn(entidad);
        EtapasAdmision resultado = frm.getIdByText(testId.toString());
    ...
    void getIdByText_IdNulo_RetornaNull() {
        ...
        verify(etapasAdmisionDAO, never()).leer(any());
    }

    @Test
    void getIdByText_FormatoInvalido_RetornaNull() {
        assertNull(frm.getIdByText("no-es-uuid-valido"));
        verify(etapasAdmisionDAO, never()).leer(any());
    }
    ...
    void createNewEntity_CamposInicializadosCorrectamente() {
        ...
        EtapasAdmision nuevo = frm.getRegistro();
        assertNotNull(nuevo);
        assertEquals("", nuevo.getNombre());
        assertEquals("", nuevo.getDescripcion());
    ...
    void inicializar_InvocaRegistros() {
        when(etapasAdmisionDAO.findRange(0, 5)).thenReturn(List.of(entidad));
        frm.inicializar();
        verify(etapasAdmisionDAO).findRange(0, 5);
    ...
    void btnGuardarHandler_Valido_CreaRefrescaYLimpia() {
        ...
        when(etapasAdmisionDAO.findRange(anyInt(), anyInt())).thenReturn(List.of());
        frm.setRegistro(entidad);
        frm.btnGuardarHandler(null);
        verify(etapasAdmisionDAO).crear(entidad);
        assertNull(frm.getRegistro());
        assertEquals(ESTADO_CRUD.NADA, frm.getEstado());
        verificarSeveridad(FacesMessage.SEVERITY_INFO);
    }
    ...
    void btnGuardarHandler_RegistroNulo_ErrorSinLlamarDao() {
        ...
        frm.btnGuardarHandler(null);
        verify(etapasAdmisionDAO, never()).crear(any());
    ...
    void btnGuardarHandler_Excepcion_ErrorSinRefrescar() {
        frm.setRegistro(entidad);
        doThrow(new RuntimeException("Fallo BD")).when(etapasAdmisionDAO).crear(any());
        frm.btnGuardarHandler(null);
        verify(etapasAdmisionDAO, never()).findRange(anyInt(), anyInt());
        verificarSeveridad(FacesMessage.SEVERITY_ERROR);
    }
    ...
    void btnModificarHandler_Valido_ActualizaRefrescaYLimpia() {
        ...
        verify(etapasAdmisionDAO).actualizar(entidad);
        assertNull(frm.getRegistro());
        verificarSeveridad(FacesMessage.SEVERITY_INFO);
    }
    ...
    void btnModificarHandler_RegistroInvalido_ErrorSinActualizar(EtapasAdmision registroInvalido) {
        ...
        frm.btnModificarHandler(null);
        verify(etapasAdmisionDAO, never()).actualizar(any());
        verificarSeveridad(FacesMessage.SEVERITY_ERROR);
    }
    ...
    void btnEliminarHandler_Valido_EliminaRefrescaYLimpia() {
        ...
        verify(etapasAdmisionDAO).eliminar(entidad);
        assertNull(frm.getRegistro());
        verificarSeveridad(FacesMessage.SEVERITY_INFO);
    }
    ...
    void btnEliminarHandler_RegistroInvalido_ErrorSinEliminar(EtapasAdmision registroInvalido) {
        frm.setRegistro(registroInvalido);
        frm.btnEliminarHandler(null);
        verify(etapasAdmisionDAO, never()).eliminar(any());
        verificarSeveridad(FacesMessage.SEVERITY_ERROR);
    }
    ...
    void paginacion_CambioValores_UsaOffsetCorrecto() {
        ...
        frm.setPageSize(10);
        when(etapasAdmisionDAO.findRange(20, 10)).thenReturn(List.of());
        frm.inicializarRegistros();
        verify(etapasAdmisionDAO).findRange(20, 10);
    }
}
